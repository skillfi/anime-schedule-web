...
import {Episode, EpisodeTimeLine, IAnime} from "../types/types";
...
function update(id: string, data: FormData) {
    const promise = http.put(new Url('/anime/', id).path, data);
    return from(promise)
}

function getTimeline(){
    const request = http.get<{data: EpisodeTimeLine[]}>(new Url('/anime/timeline').path)
    return from(request)
}
...
function generateAnime(data: FormData) {
    const request = http.post<{ data: IAnime }>(new Url('/generate/anime').path, data)
    return from(request)
}
...
function uploadEpisode(episode_id: string){
    const request = http.put<{data: Episode}>(new Url('/episode/', episode_id + '/upload').path)
    return from(request)
}
