...
export default class AnimeComponent extends React.Component<PageProps, { anime: IAnime, bookmarks: string[] }> {
    ...
    constructor(props: PageProps) {
        ...
        this.state = {
            anime: {
                ...
                time: 25, image: '', subscribe: false, tags: [], full_title: '', description: [], aid: 0, title_en: '', title_jp: ''
            }, bookmarks: []
        }
    ...
    render() {
        ...
        this.state.anime.tags.map((tag, index) => {
                    ...
                    <Grid container spacing={2}>
                                    ...
                                    <Grid item xs={3} md>
                                        <Item>
                                            <Typography fontSize={20} textAlign={'center'} fontFamily={'Cambria Math'}>
                                                {this.state.anime.description.map((text)=>{
                                    ...
                                    <Grid item xs={2} sx={{height: 50}}>
                                        <Item elevation={3}>
                                            <Typography textAlign={'center'} fontFamily={'Cambria Math'} fontSize={20}>
                                                <ChipTagComponent
                                                    tag={this.getEpisode(this.state.anime.episode).toString()}/> <ChipTagComponent tag={this.getNoEpisode(this.state.anime.episode).toString()}/> / <ChipTagComponent tag={this.state.anime.episodes.toString()}/>
                                            </Typography>
                                        </Item>
                                    </Grid>
                                </Grid>
                            ...
                            <Box>
                                ...
                                <Item>
                                    <Typography variant={'subtitle2'} fontSize={18} display={'block'}>
                                        {this.state.anime.description.map((text)=>{
                                            if (text.includes('* Based')){
                                                return text
                                            }
                                        })}
                                    </Typography>
                                    <Typography variant={'subtitle2'} fontSize={18}>
                                        {this.state.anime.description[1]}
                                    </Typography>
                        ...
                        <Grid item>
                            <TableEpisodeComponent rows={this.state.anime.episode}
                                                   columns={['Episode_Id', 'Air_Date', 'Title', 'Full_Titles', 'Viewed']}
                                                  sx={{width: Tools.getWindowDimension().minWidth * 0.9}}/>
                        </Grid>
                    </Grid>
                </Box>
            </Box>
